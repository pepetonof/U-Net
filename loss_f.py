...
class ComboLoss(nn.Module):
    ...
    def forward(self, inputs:torch.Tensor, targets:torch.Tensor):
        ...
        # Avoid extreme cases ln(0) and ln (1)
        inputs_soft = torch.clamp(inputs_soft, self.eps, 1.0 - self.eps) 
...
def combo_loss(inputs:torch.Tensor, targets:torch.Tensor):
    Closs= ComboLoss(alpha=0.5, beta=0.4)#beta=0.7
...
def hdistance_loss(inputs:torch.Tensor, targets:torch.Tensor):
    ...
    #hdistance=torch.Tensor([hdistance])
    #hdistance=torch.from_numpy(hdistance)
