...
def train(train_loader, model, optimizer, loss_fn, scaler, epoch, DEVICE):
...
def validate(val_loader, model, loss_fn, epoch, DEVICE):
    metric_monitor=MetricMonitor()
    model.eval()
...
def train_and_validate(model, train_loader, val_loader, 
    ...
    for epoch in range(1,num_epochs+1):
        losst, dicet = train(train_loader, model, 
                      optimizer, loss_fn, scaler, epoch, DEVICE)
        train_loss.append(losst)
        train_dice.append(dicet)
        
        lossv, dicev = validate(val_loader, model, loss_fn, epoch, DEVICE)
