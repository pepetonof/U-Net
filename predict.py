...
import numpy as np
import torchvision
from train_valid import MetricMonitor
...
def test(test_loader, model, loss_fn, 
         save_imgs=False, ruta="saved_images/", device="cuda"):
    ...
    loss_out=running_loss/len(test_loader)
    
    """Monitor Dice for Test Set Validation"""
    metric_monitor.update("Loss", loss_out)
    # stream.set_description("Testing. \t\t{metric_monitor}".format(metric_monitor=metric_monitor))
    print("Testing.\t{metric_monitor}".format(metric_monitor=loss_out.item()))
...
def overlay_imgs(inputs, masks, preds, alpha=0.3):
